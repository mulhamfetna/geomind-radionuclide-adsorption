"""GEOMIND-R Virtual Chemistry Lab — Phase 1: the Screening Workbench.

A local Gradio app over the tested `app.engine` service layer. It is a glass box,
not an oracle: every Sr K_D prediction renders with its tri-colour confidence flag
(validated / exploratory / unsupported), its plain-language reason, and its
provenance. Runs locally only — no network, to keep pre-publication work off any
server.

Run:  python -m app.lab      (opens http://127.0.0.1:7860)
"""
...
def _predict_handler(al_iv_mmol_g: float, structural_class: str, adsorbate: str):
    """UI adapter for a single Sr K_D prediction. Returns (label, detail_markdown).

    label   — the badge + headline value (identity of the flag, never colour-alone).
    detail  — the reason, uncertainty, equation and provenance, as Markdown.
    """
    card = E.predict_kd(al_iv_mmol_g, structural_class=structural_class,
                        adsorbate=adsorbate)
    ...
    if card.uncertainty is not None:
        lines.append(f"**Uncertainty (LOO-CV RMSE):** ±{card.uncertainty:,.0f} "
                     f"{card.unit}")
    lines += [
        "",
        f"**Why:** {card.why}",
...
def _screen_handler(b_L_mg: float, c0_mg_L: float):
    """UI adapter for the saturation advisor. Returns Markdown."""
...
def build_app() -> "gr.Blocks":
    """Assemble the Workbench Blocks app (no launch)."""
    with gr.Blocks(title="GEOMIND-R Virtual Lab — Screening Workbench") as demo:
        gr.Markdown(
            "# GEOMIND-R Virtual Chemistry Lab — Screening Workbench\n"
            "A **glass box, not an oracle.** Every prediction shows its confidence "
            "flag, its reason, and its source. Predictions are trustworthy only for "
            "**metakaolin geopolymers / Sr** inside the trained [Al^IV] range "
            "(3.45–4.77 mmol g⁻¹); everything else is flagged. *Local, "
            "pre-publication — no data leaves this machine.*"
        )
        ...
        with gr.Tab("Sr K_D prediction"):
            with gr.Row():
                with gr.Column():
                    ...
                    sclass = gr.Dropdown(_CLASSES, value=_CLASSES[0],
            ...
            go.click(_predict_handler, [al_iv, sclass, ads],
                     [out_label, out_detail])

        with gr.Tab("Saturation advisor (θ = 1 − R_L)"):
            gr.Markdown("Does a reported Langmuir fit actually reach the plateau? "
                        "Enter its *b* and the highest C₀ tested.")
            with gr.Row():
...
def main():  # pragma: no cover — entry point, not unit-tested
    build_app().launch()
